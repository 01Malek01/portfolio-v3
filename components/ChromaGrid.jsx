...
const ChromaGrid = ({ items, className = '', radius = 300, damping = 0.45, fadeOut = 0.6, ease = 'power3.out' }) => {
  ...
  return (
    <div
      ...
      className={`relative w-full h-full flex flex-wrap justify-center items-start gap-3 py-5 rounded-lg ${className}`}
      style={{
        '--r': isMobile ? '1000px' : `${radius}px`,
        '--x': '50%',
        '--y': '50%'
      }}
    >
      {data.map((c, i) => (
        <article
          ...
          // onClick={() => handleCardClick(c.liveLink)}
          className="group relative flex w-[30rem] rounded-[20px] overflow-hidden border-2 border-transparent transition-colors duration-300 cursor-pointer"
          style={{
        ...
        >
          ...
          {/* <div className="relative z-10 flex-1 p-[10px] box-border">
            <img src={c.image} alt={c.name} loading="lazy" className="w-full h-full object-cover rounded-[10px]" />
          </div>
